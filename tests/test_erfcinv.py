...
from certsf import SFResult, erfc, erfcinv, erfinv, mcp_server
...
def test_erfcinv_one():
    result = erfcinv("1", dps=80, mode="high_precision")

    assert result.function == "erfcinv"
    assert _mp_number(result.value) == 0


@pytest.mark.parametrize("x", ["0.5", "1.5", "0.1", "1.9"])
def test_erfc_of_erfcinv_matches_input(x):
    inverse = erfcinv(x, dps=80, mode="high_precision")
    composed = erfc(inverse.value, dps=80, mode="high_precision")

    _assert_close(_mp_number(composed.value), mp.mpf(x), mp.mpf("1e-75"))


@pytest.mark.parametrize("x", ["0.5", "1.5", "0.1", "1.9"])
def test_erfcinv_matches_erfinv_relation(x):
    inverse = erfcinv(x, dps=80, mode="high_precision")
    via_erfinv = erfinv(str(1 - mp.mpf(x)), dps=80, mode="high_precision")

    _assert_close(_mp_number(inverse.value), _mp_number(via_erfinv.value), mp.mpf("1e-75"))


def test_erfcinv_monotonic_orientation():
    positive = erfcinv("0.5", dps=80, mode="high_precision")
    negative = erfcinv("1.5", dps=80, mode="high_precision")

    assert _mp_number(positive.value) > 0
    assert _mp_number(negative.value) < 0


@pytest.mark.parametrize("x", ["0", "2", "-0.1", "2.1"])
def test_certified_erfcinv_rejects_closed_or_outside_domain(x):
    result = erfcinv(x, dps=50, mode="certified")
    ...
    assert result.diagnostics["certificate_scope"] == "arb_erfcinv_via_erfinv"
    assert result.diagnostics["domain"] == "real_x_in_open_interval_0_2"
    assert "error" in result.diagnostics


def test_certified_erfcinv_rejects_complex_input():
    result = erfcinv("0.5+0.25j", dps=50, mode="certified")
...
def test_erfcinv_auto_dispatch_behavior():
    fast = erfcinv("0.5", dps=15, mode="auto")
    high_precision = erfcinv("0.5", dps=50, mode="auto")
    certified = erfcinv("0.5", dps=50, mode="auto", certify=True)
...
def test_erfcinv_all_modes_return_sfresult():
    for mode in ("fast", "high_precision", "certified", "auto"):
        result = erfcinv("0.5", dps=40, mode=mode)
        assert isinstance(result, SFResult)
        assert result.function == "erfcinv"


def test_special_erfcinv_mcp_tool_matches_python_api():
    payload = mcp_server.special_erfcinv("0.5", dps=50, mode="high_precision")

    assert payload == erfcinv("0.5", dps=50, mode="high_precision").to_mcp_dict()
    assert payload["function"] == "erfcinv"
    assert payload["backend"] == "mpmath"


@pytest.mark.parametrize("x", ["0.5", "1.5", "0.1", "1.9"])
def test_certified_erfcinv_residual_contains_zero(x):
    flint = pytest.importorskip("flint")
    result = erfcinv(x, dps=90, mode="certified")
    ...
    try:
        ...
        residual = y_ball.erfc() - flint.arb(x)
        assert residual.contains(0)
    finally:
        flint.ctx.prec = old_prec


def test_certified_erfcinv_scope_and_diagnostics_are_narrow():
    result = erfcinv("0.5", dps=60, mode="certified")
    _require_certified(result)
    ...
    if result.diagnostics["certificate_scope"] == "direct_arb_erfcinv":
        ...
        assert "real principal erfcinv(x)" in result.diagnostics["certification_claim"]
    else:
...
def test_erfcinv_via_erfinv_fallback_records_certified_method(monkeypatch):
    pytest.importorskip("flint")
    monkeypatch.setattr(arb_backend, "_direct_arb_erfcinv_method", lambda _argument: None)

    result = arb_backend.arb_erfcinv("0.5", dps=50)
