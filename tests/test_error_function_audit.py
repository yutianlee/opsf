...
DOC_EXPECTATIONS = {
    ...
    "docs/audit/erfinv_inverse.md": (
        ...
        "`pypi-smoke.yml` defaults to `0.3.0`",
...
def test_pypi_smoke_covers_error_function_release_surface():
    ...
    assert 'default: "0.3.0"' in text
    assert "inputs.version || '0.3.0'" in text
...
def test_release_policy_and_pypi_smoke_final_guardrails_remain_current():
    ...
    assert 'default: "0.3.0"' in smoke
    assert "inputs.version || '0.3.0'" in smoke
