"""Custom positive-real reciprocal-gamma method via certified loggamma exponentiation."""
...
_CERTIFICATE_SCOPE = "rgamma_positive_real_stirling_recip"
_CERTIFICATE_LEVEL = "custom_asymptotic_bound"
_AUDIT_STATUS = "theorem_documented"
_CERTIFICATION_CLAIM = "certified positive-real reciprocal gamma enclosure via certified loggamma exponentiation"
_DOMAIN = "positive_real_x_ge_20"
_FORMULA = "rgamma=exp(-loggamma)"
_METHOD = "stirling_recip_rgamma"
_BACKEND = "certsf+python-flint"


def rgamma_stirling_recip(x: Any, *, dps: int = 50) -> SFResult:
    """Certified positive-real ``rgamma(x)`` via an internal loggamma Arb ball."""
    ...
    try:
        ...
        rgamma_ball = (-loggamma_ball).exp()
        if not bool(rgamma_ball.is_finite()):
            return _unavailable(
                requested_dps,
                "stirling_recip rgamma exponentiation produced a non-finite enclosure.",
                working_dps=_working_dps(loggamma, requested_dps),
                diagnostics={"working_precision_bits": bits},
            )

        abs_error_bound = _safe_positive_string(rgamma_ball.rad(), requested_dps, flint)
        propagated_tail = rgamma_ball.abs_upper() * loggamma["tail_bound"]
        ...
        diagnostics.update(
            {
                ...
                "exp_radius": _safe_positive_string(rgamma_ball.rad(), requested_dps, flint),
                "propagated_error_bound": _safe_positive_string(propagated_tail, requested_dps, flint),
            }
        )
        return make_result(
            function="rgamma",
            value=_arb_value_string(rgamma_ball),
            abs_error_bound=abs_error_bound,
            rel_error_bound=_relative_error_bound(abs_error_bound, rgamma_ball),
            certified=True,
            method=_METHOD,
            backend=_BACKEND,
        ...
        return _unavailable(
            requested_dps,
            f"stirling_recip rgamma method failed cleanly: {exc}",
            working_dps=_working_dps(loggamma, requested_dps),
            diagnostics={"working_precision_bits": bits},
        )
    finally:
