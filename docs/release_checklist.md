...
- [ ] TestPyPI staging is required or strongly recommended under
  ...
  0.3 line.
- [ ] Manually run `publish-testpypi` with `ref=v0.3.0` and
  `confirm=publish-testpypi` before the real PyPI release.
- [ ] Keep the TestPyPI confirm guard in `publish-testpypi.yml`.
- [ ] Publish the real PyPI release through the protected `publish-pypi`
  workflow from the normal GitHub release.
...
- [ ] Custom methods remain explicit only:
  `loggamma(method="stirling")`,
  `loggamma(method="stirling_shifted")`,
  `loggamma(method="certified_auto")`,
  `gamma(method="stirling_exp")`, and
  `rgamma(method="stirling_recip")`.
- [ ] Complex Stirling, reflection formula certification, near-pole behavior
  support, gamma-ratio asymptotics, and beta asymptotics are excluded.
- [ ] Parabolic-cylinder wrappers remain `experimental_formula`.
- [ ] Release copy does not broaden certification claims.
- [ ] `python scripts/check_release_version.py v0.3.0` passes.
...
- [ ] `pyproject.toml` version is `0.3.0a4`.
- [ ] `CITATION.cff` version is `0.3.0-alpha.4`.
- [ ] `CITATION.cff` date-released is `2026-05-10`.
- [ ] `CHANGELOG.md` records the explicit positive-real `gamma`
  `method="stirling_exp"` alpha.
- [ ] `docs/release-0.3.0-alpha.4.md` is reviewed.
- [ ] Publish workflow defaults point at `v0.3.0-alpha.4`.
- [ ] PyPI smoke workflow remains pinned to `0.3.0a3` until `0.3.0a4` is
  published and smoke verification passes.
- [ ] GitHub release for `v0.3.0-alpha.4` is marked prerelease.
- [ ] TestPyPI staging is skipped unless release validation identifies
  packaging or workflow risk under `release_policy.md`.
- [ ] Publish the real PyPI prerelease through the protected `publish-pypi`
  workflow from the GitHub prerelease.
- [ ] Run `pypi-smoke` against `0.3.0a4` after real PyPI publication.
- [ ] No `src/` runtime implementation changes are included in the
  release-planning PR.
- [ ] No public-wrapper changes are included.
- [ ] Release copy says this alpha packages the explicit positive-real
  `gamma` `method="stirling_exp"` for finite real `x >= 20`.
- [ ] Release copy says `method="stirling_exp"` computes `gamma(x)` by
  exponentiating a certified positive-real `loggamma` Arb enclosure.
- [ ] Release copy records
  `certificate_scope="gamma_positive_real_stirling_exp"` and runtime result
  method `method="stirling_exp_gamma"`.
- [ ] Default certified `gamma` remains the direct Arb path.
- [ ] `method=None` and `method="auto"` remain unchanged.
- [ ] `method="stirling_exp"` is explicit only and is not automatic default
  selection.
- [ ] Complex `gamma`, reflection formula certification, near-pole behavior,
  real `x < 20`, real `x <= 0`, gamma-ratio asymptotics, and beta asymptotics
  are excluded.
- [ ] Parabolic-cylinder wrappers remain `experimental_formula`.
- [ ] Release copy does not broaden certification claims.
- [ ] `python scripts/check_release_version.py v0.3.0-alpha.4` passes.
- [ ] `python -m ruff check .` passes.
- [ ] `python -m mypy` passes.
- [ ] `python -m pyright src` passes.
- [ ] `python -m pytest` passes.
...
- Git tag `v0.3.0-alpha.4` points at
  `8f23c6df32a065fc5e9aa8e702dc68fcefe2d094`.
- GitHub release `v0.3.0-alpha.4` is marked prerelease and is not a draft.
...
- Real PyPI publish run `25661919365` passed and PyPI confirms
  `certsf 0.3.0a4`.
- Final `pypi-smoke` run `25663130452` passed against `0.3.0a4` across base,
...
## v0.3.0-alpha.3 Checklist

- [ ] `pyproject.toml` version is `0.3.0a3`.
- [ ] `CITATION.cff` version is `0.3.0-alpha.3`.
- [ ] `CITATION.cff` date-released is `2026-05-10`.
- [ ] `CHANGELOG.md` records the `certified_auto` preselection optimization
  alpha.
- [ ] `docs/release-0.3.0-alpha.3.md` is reviewed.
- [ ] Publish workflow defaults point at `v0.3.0-alpha.3`.
- [ ] PyPI smoke workflow remains pinned to `0.3.0a2` until `0.3.0a3` is
  published and smoke verification passes.
- [ ] GitHub release for `v0.3.0-alpha.3` is marked prerelease.
- [ ] TestPyPI staging is skipped unless release validation identifies
  packaging or workflow risk under `release_policy.md`.
- [ ] Publish the real PyPI prerelease through the protected `publish-pypi`
  workflow from the GitHub prerelease.
- [ ] Run `pypi-smoke` against `0.3.0a3` after real PyPI publication.
- [ ] No `src/` runtime implementation changes are included in the
  release-planning PR.
- [ ] No public-wrapper changes are included.
- [ ] Release copy says this alpha packages the explicit `certified_auto`
  preselection optimization for existing `loggamma`.
- [ ] Default certified `loggamma` remains the direct Arb path.
- [ ] `method=None` and `method="auto"` remain unchanged.
- [ ] `method="certified_auto"` is explicit only and may select direct Arb,
  `method="stirling"`, or `method="stirling_shifted"`.
- [ ] Release copy says preselection avoids unnecessary full custom candidate
  evaluations without changing default dispatch.
- [ ] Complex `loggamma` branches, complex Stirling, `x < 20` custom
  Stirling, `x <= 0` custom Stirling, gamma-ratio asymptotics, and beta
  asymptotics are excluded.
- [ ] Parabolic-cylinder wrappers remain `experimental_formula`.
- [ ] Release copy does not broaden certification claims.
- [ ] `python scripts/check_release_version.py v0.3.0-alpha.3` passes.
- [ ] `python -m ruff check .` passes.
- [ ] `python -m mypy` passes.
- [ ] `python -m pyright src` passes.
- [ ] `python -m pytest` passes.
...
- [ ] `CITATION.cff` version is `0.3.0-alpha.2`.
- [ ] `CITATION.cff` date-released is `2026-05-10`.
- [ ] `CHANGELOG.md` records the explicit `certified_auto` alpha.
- [ ] `docs/release-0.3.0-alpha.2.md` is reviewed.
- [ ] Publish workflow defaults point at `v0.3.0-alpha.2`.
- [ ] PyPI smoke workflow remains pinned to `0.3.0a1` until `0.3.0a2` is
  published and smoke verification passes.
- [ ] GitHub release for `v0.3.0-alpha.2` is marked prerelease.
- [ ] TestPyPI staging is skipped unless release validation identifies
  packaging or workflow risk under `release_policy.md`.
- [ ] Publish the real PyPI prerelease through the protected `publish-pypi`
  workflow from the GitHub prerelease.
- [ ] Run `pypi-smoke` against `0.3.0a2` after real PyPI publication.
- [ ] No `src/` runtime implementation changes are included in the
  release-planning PR.
- [ ] No public-wrapper changes are included.
- [ ] The public wrapper surface remains the same as `0.3.0-alpha.1`.
- [ ] Release copy says the new functionality is explicit certified method
  selection for existing `loggamma` via `method="certified_auto"`.
- [ ] Default certified `loggamma` remains the direct Arb path.
- [ ] `method=None` and `method="auto"` remain unchanged.
- [ ] `method="certified_auto"` is explicit only and may select direct Arb,
  `method="stirling"`, or `method="stirling_shifted"`.
- [ ] Complex `loggamma` branches, `x < 20` custom Stirling, `x <= 0` custom
  Stirling, gamma-ratio asymptotics, and beta asymptotics are excluded.
- [ ] Parabolic-cylinder wrappers remain `experimental_formula`.
- [ ] Release copy does not broaden certification claims.
- [ ] `python scripts/check_release_version.py v0.3.0-alpha.2` passes.
- [ ] `python -m ruff check .` passes.
- [ ] `python -m mypy` passes.
- [ ] `python -m pyright src` passes.
- [ ] `python -m pytest` passes.
...
- Git tag `v0.3.0-alpha.2` points at
  `1d51181e3b90c379e1ef543aca85e008cdf090d6`.
- GitHub release `v0.3.0-alpha.2` is marked prerelease and is not a draft.
- TestPyPI staging was skipped under `release_policy.md` because this was a
  routine feature alpha without packaging, dependency, or workflow-risk changes.
- Real PyPI publish run `25627010550` passed and PyPI confirms
  `certsf 0.3.0a2`.
- Initial `pypi-smoke` runs `25627041563` and `25627081269` captured PyPI
  edge-cache propagation failures where selected runners saw only versions
  through `0.3.0a1`.
- Final `pypi-smoke` run `25627550209` passed against `0.3.0a2` across base,
  certified, and MCP-certified install paths on Python 3.10, 3.11, and 3.12.
- Wheel SHA256:
